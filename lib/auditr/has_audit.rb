...
#              child_audit_entries = (child_audit_entries or child_record.audit_entries) unless child_record.audit_entries.blank?         
              query = query.or(t[:item_type].eq(child_record.class.name).and(t[:item_id].eq(child_record.id)))
...
#        return child_audit_entries
      ...
      def record_create
        if switched_on?
          data = {
            :event     => 'create',
            :entry     => "#{self.class.name.titleize} created",
            :severity  => 'info',
            :user      => Auditr.current_user
          }
      ...
      def record_update
        if switched_on?
          data = {
            :event     => 'update',
            :entry     => "#{self.class.name.titleize} updated",
            :severity  => 'info',
      ...
      def record_destroy
        if switched_on? and not new_record?
          audit_entry_class.create merge_metadata(:item_id   => self.id,
                                              ...
                                              :event     => 'destroy',
                                              :entry     => "#{self.class.name.titleize} deleted",
                                              :severity  => 'warning',
#                                              :object    => object_to_string(item_before_change),
                                              :user      => Auditr.current_user)
        end
